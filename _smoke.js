...
const BASE = `http://localhost:${process.env.PORT || 3017}`;
const results = [];
let token = null;
...
(async () => {
  // Register
  const reg = await req('POST', '/api/auth/register', { username: 'smoke' + Date.now(), email: `smoke${Date.now()}@t.com`, password: 'Test123456', birthDate: '1990-01-01', state: 'CA' });
  ...
  await check('blackjack full round', async () => {
    ...
    const gameId = r.data?.gameId; if (!gameId) return r;
    // exercise DOUBLE when the hand is still active
